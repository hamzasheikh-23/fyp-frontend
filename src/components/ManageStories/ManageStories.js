...
class ManageStories extends Component {
  componentDidMount() {
    const ngo_id = localStorage.getItem("userID");
    console.log("/api/getStoriesOfNgo/" + ngo_id);
    axios
      .get("api/getStoriesOfNgo/" + ngo_id)
      .then((res) => {
  ...
  updateList = () => {
    axios
      .get(`/api/getStoriesOfNgo/${localStorage.getItem("userID")}`)
      .then((res) => {
        const stories = res.data.filter(
          (story) => story.status === "Pending" || story.status === "Approved"
        );
                ...
                {this.state.arrayforcards.map((data) => (
                  <div className="">
                    <Card className="bg-dark text-white  ">
                      <Card.Img
                        src={`http://localhost:8000/storage/cover_images/${data.image}`}
                        alt="Card image"
                      ...
                      <Button
                        variant="primary"
                        className="bold-text"
                        onClick={() => this.deleteCard(data.story_id)}
                      >
                        {"Delete Story"}
                      </Button>
                    </Card>
